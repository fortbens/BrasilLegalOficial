...
export const RaioXImovel: React.FC<RaioXImovelProps> = ({
  ...
  // Sincronização com o histórico do navegador para permitir 'Voltar' sem sair da página
  useEffect(() => {
    if (typeof window === 'undefined') return;

    // Inicializa o estado atual no histórico da aba
    if (!window.history.state || typeof window.history.state.raioXEtapa !== 'number') {
      window.history.replaceState({ raioXEtapa: 1 }, '', window.location.href);
    }
    ...
    const handlePopState = (event: PopStateEvent) => {
      ...
      if (event.state && typeof event.state.raioXEtapa === 'number') {
        setEtapa(event.state.raioXEtapa);
      } else {
        // Se voltou além do início do Raio-X, mantém na etapa 1 para nunca fechar a página
        setEtapa(1);
      }
  ...
  const avancarParaEtapa = (novaEtapa: number) => {
    setErroValidacao('');
    if (typeof window !== 'undefined' && !isNavigatingHistoryRef.current) {
      window.history.pushState({ raioXEtapa: novaEtapa }, '', window.location.pathname + '#raio-x');
    }
    setEtapa(novaEtapa);

    // Rola suavemente até o corpo do formulário
  ...
  const handleVoltar = () => {
    setErroValidacao('');
    if (typeof window !== 'undefined' && window.history.state && window.history.state.raioXEtapa > 1) {
      window.history.back();
    } else {
                    ...
                    {opcoesProblema.map((opt) => {
                      ...
                      return (
                        <button
                          ...
                          className={`group p-3.5 rounded-xl text-left border transition-all flex items-center justify-between cursor-pointer ${
                            isSelected
                              ? 'border-[#2E3192] bg-indigo-50/80 shadow-xs ring-1 ring-[#2E3192]'
                              : 'border-slate-200 bg-white hover:border-[#2E3192]/40 hover:bg-slate-50'
                          }`}
                        >
                          <div className="flex items-start gap-3 w-full">
                            ...
                            <div className="min-w-0 pt-0.5 flex-1 pr-2">
                              <p className={`text-xs sm:text-sm font-semibold leading-snug ${
                                isSelected ? 'text-[#1C1E63] font-bold' : 'text-slate-800'
                              }`}>
                                {opt.label}
                              </p>
                            </div>
                            {isSelected && (
                              <div className="w-5 h-5 rounded-full bg-[#2E3192] text-white flex items-center justify-center shrink-0 mt-0.5">
                    ...
                    {opcoesObjetivo.map((opt) => {
                      ...
                      return (
                        <button
                          ...
                          className={`group p-4 rounded-xl text-left border transition-all flex items-start justify-between cursor-pointer ${
                            isSelected
                              ? 'border-[#2E3192] bg-indigo-50/80 shadow-xs ring-1 ring-[#2E3192]'
                              : 'border-slate-200 bg-white hover:border-[#2E3192]/40 hover:bg-slate-50'
                          }`}
                    ...
                    {opcoesConstrucao.map((opt) => {
                      ...
                      return (
                        <button
                          ...
                          className={`group w-full p-4 rounded-xl text-left border transition-all flex items-center justify-between cursor-pointer ${
                            isSelected
                              ? 'border-[#2E3192] bg-indigo-50/80 shadow-xs ring-1 ring-[#2E3192]'
                              : 'border-slate-200 bg-white hover:border-[#2E3192]/40 hover:bg-slate-50'
                          }`}
